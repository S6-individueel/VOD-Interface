...
export default function Patch() {
    ...
    const handleOnClick = useCallback(() => history.push('/config/profile'), [history]);

    const [user, setUser] = useState({email: '', password: '', displayName: ''});
    const [errors, setErrors] = useState({errors: []});
    const [errorMessage, setErrorMessage] = useState();
    const [onSubmitDisabled, setOnSubmitDisabled] = useState(true);

    useEffect(() => {
        Authentication.getUserById()
            .then(response => {
                setUser({email: response.data.email, password: response.data.password, displayName: response.data.displayName})
            })
    },[]);

    const onEmailChange = (e) => {
        setUser({...user, email: e.target.value});
        setOnSubmitDisabled(false);
    ...
    const submit = () => {
        ...
        if (user.email === '') {
            ...
            Authentication.modifyUser(user)
                .then(response => {
                    handleOnClick();
                })
                .catch((error) => {
                    showValidationErr('general', 'Changes to user could not be saved!');
                    setErrorMessage({errorShow: true, errorTitle: 'User exists', errorMessage: `user with the email: ${user.email} could not be changed`});
                })
        }
                          ...
                          Privacy Policy
                                            ...
                                            <div className="text-center mt-6">
                                                <button
                                                    ...
                                                    + (onSubmitDisabled ? "bg-red-200" : "bg-blueGray-800")}
                                                    type="button"
                                                    onClick={() => submit()}
